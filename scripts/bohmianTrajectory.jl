...
function ϕ_re(t)
    (cos(t * real(λ[1])) * mode_0_re + cos(t * real(λ[2])) * mode_1_re -
     sin(t * real(λ[1])) * mode_0_im - sin(t * real(λ[2])) * mode_1_im)
end

function ϕ_im(t)
    (cos(t * real(λ[1])) * mode_0_im + cos(t * real(λ[2])) * mode_1_im +
...
function v_Bohm!(du, u, p, t)
    @unpack ϕ_re, ϕ_im = p
    x = Gridap.Point(u...)
    v = (ϕ_re(t)(x) * ∇(ϕ_im(t))(x) - ϕ_im(t)(x) * ∇(ϕ_re(t))(x)) ./
        (ϕ_re(t)(x)^2 + ϕ_im(t)(x)^2)
