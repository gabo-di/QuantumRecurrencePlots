...
function plot_comparison_1D!(fig, x, tmax, ψ_numerical, ψ_analytical)
    ...
    lines!(ax1, x, real.(ψ_analytical), color = :red,
        linestyle = :dash, linewidth = 2, label = "Analytical")
    ...
    lines!(ax2, x, imag.(ψ_analytical), color = :red,
        linestyle = :dash, linewidth = 2, label = "Analytical")
    ...
    lines!(ax3, x, abs2.(ψ_analytical), color = :red,
        linestyle = :dash, linewidth = 2, label = "Analytical")
    ...
    Label(fig[0, 1:2],
        "Solution after t = $(round(tmax, digits=3)) (Error: $(round(norm_error, digits=6)))",
        fontsize = 20)
