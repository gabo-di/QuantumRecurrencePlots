...
function linregress(X, y, weights = nothing)
    ...
    size(X, 1) == size(y, 1) ||
        throw(DimensionMismatch("size of X and y does not match in first dimension"))
    X = _append_bias_column(X)
    coeffs = if weights === nothing
        _lin_solve(X, y)
    else
        length(weights) == size(y, 1) ||
            throw(DimensionMismatch("size of y and weights does not match in first dimension"))
        W = Diagonal(weights)
        _lin_solve(X, y, W)
    end
    return coeffs
